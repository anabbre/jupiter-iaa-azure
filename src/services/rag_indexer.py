    ...
    def __post_init__(self):
        ...
        if self.chunk_configs is None:
            self.chunk_configs = {
                ...
                "terraform": {"chunk_size": 1800, "chunk_overlap": 300}, # Código tf en bloques completos
                "markdown": {"chunk_size": 1000, "chunk_overlap": 150},  # Docs técnicas
                "example": {"chunk_size": 2000, "chunk_overlap": 400}    # Ejemplos completos
            }
...
class MetadataEnricher:
    ...
    def extract_section_from_pdf(page_content: str) -> Optional[str]:
        ...
        for line in lines[:5]:  # Buscar en las primeras 5 líneas
            ...
            if line and (line.isupper() or re.match(r'^#+\s+', line) or re.match(r'^\d+\.', line)):
    ...
    def extract_code_quality_metrics(content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "complexity_score": content.count('{') + content.count('for_each') + content.count('count'),
...
class DocumentLoader:
    ...
    def load_pdfs(self, pdf_dir: Path) -> List[Document]:
        ...
        logger.info(f"📄 Cargando {len(pdf_files)} PDFs", source="qdrant", pdf_count=len(pdf_files))
        ...
        for pdf_file in pdf_files:
            try:
                ...
                if not valid_pages:
                    logger.warning(f"⏭️ PDF sin contenido válido", source="qdrant", file=pdf_file.name)
                    continue  
                ...
                logger.info(f"📑 PDF dividido en chunks", source="qdrant", file=pdf_file.name, pages=len(valid_pages),chunks=len(chunks))

                # Enriquecer metadatos de cada chunk
                for i, chunk in enumerate(chunks):
                    # Extraer sección del contenido
                    section = self.metadata_enricher.extract_section_from_pdf(chunk.page_content)
                ...
                logger.info(f"✅ PDF procesado", source="qdrant", file=pdf_file.name, chunks_indexed=len([c for c in chunks if not self.deduplicator.is_duplicate(c.page_content, {"source": str(pdf_file)})]))
            except Exception as e:
                logger.error(f"❌ Error cargando PDF", source="qdrant", file=pdf_file.name, error=str(e))
    ...
    def load_markdown_files(self, md_dir: Path, is_example: bool = False) -> List[Document]:
        ...
        logger.info(f"📝 Cargando {len(md_files)} archivos Markdown", source="qdrant", md_count=len(md_files), is_example=is_example)
        ...
        for md_file in md_files:
            try:
                ...
                for i, chunk in enumerate(chunks):
                    if not self.deduplicator.is_duplicate(
                        chunk.page_content, 
                        {"source": str(md_file), "chunk_id": i}
                    ):
                        # Extraer título de la sección
                        section_match = re.search(r'^#+\s+(.+)$', chunk.page_content, re.MULTILINE)
                ...
                logger.info(f"✅ MD procesado", source="qdrant", file=md_file.name, chunks_indexed=len(chunks))
            except Exception as e:
                logger.error(f"❌ Error cargando MD", source="qdrant", file=md_file.name, error=str(e))
        ...
        try:
            ...
            for ex in examples:
                ...
                if ex_path.is_file() and ex_path.suffix == ".pdf":
                    ...
                    # Marcar como ejemplo 
                    docs = (
                        self.load_terraform_files(ex_path, is_example=True) + 
                ...
                logger.info(f"✅ Ejemplo '{ex.get('id')}' cargado", source="qdrant", docs_count=len(docs))
        except FileNotFoundError:
            logger.warning(f"⚠️ Manifest no encontrado", source="qdrant", path=str(self.config.manifest_path))
        except Exception as e:
...
class QdrantIndexer:
    ...
    def _create_client(self) -> QdrantClient:
        ...
        try:
            ...
            logger.info("✅ Conexión Qdrant establecida", source="qdrant",url=self.config.qdrant_url)
            return client
        except Exception as e:
    ...
    def index_all(self, recreate_collections: bool = False):
        ...
        print(f"   • PDFs: {self.config.chunk_configs['pdf']['chunk_size']} chars/chunk")
        print(f"   • Terraform: {self.config.chunk_configs['terraform']['chunk_size']} chars/chunk")
        print(f"   • Markdown: {self.config.chunk_configs['markdown']['chunk_size']} chars/chunk")
        print(f"   • Ejemplos: {self.config.chunk_configs['example']['chunk_size']} chars/chunk")
        print("=" * 80 + "\n")
        
        logger.info("🚀 Inicio de indexación optimizada", 
                   source="qdrant", 
        ...
        try:
            ...
            print(f"   ⏭️  Duplicados eliminados:    {dedup_stats['duplicates_removed']:>6}")
            print(f"   📈 Tasa de dedup:            {dedup_stats['deduplication_rate']:>5.1f}%")
            print()
            print(f"⏱️  Tiempo total:               {duration:>6.2f}s")
            print(f"⚡ Velocidad:                  {total_docs/duration if duration > 0 else 0:>6.1f} chunks/s")
            print("=" * 80 + "\n")
            logger.info("✅ Indexación completa", source="qdrant",total_docs=total_docs, duration=f"{duration:.2f}s",dedup_stats=dedup_stats,stats=stats)
        except Exception as e:
            duration = time.time() - start_time
            logger.error("❌ Error en indexación", source="qdrant",duration=f"{duration:.2f}s", error=str(e))
            raise
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Indexador optimizado de documentos Terraform")
    parser.add_argument("--recreate", action="store_true", help="Recrear colecciones (borra contenido anterior)")
    ...
    parser.add_argument("--only-examples", action="store_true", help="Solo indexar ejemplos")
    parser.add_argument("--chunk-size-pdf", type=int, help="Tamaño de chunk para PDFs (default: 1200)")
    parser.add_argument("--chunk-size-tf", type=int, help="Tamaño de chunk para Terraform (default: 1800)")
